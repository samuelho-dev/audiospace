...
interface StarTypeProps {
  fill: boolean;
}

function StarType({ fill }: StarTypeProps) {
  return (
    <svg
...
function StarRating({ handleRating, existingRating }: StarRatingProps) {
  const [rating, setRating] = useState(0);
  const handleHover = (i: number) => setRating(i + 1);
      ...
      <ul className="flex justify-center">
        {Array.from({ length: 5 }).map((_, i: number) => (
          <li
            key={i}
            onMouseOver={() => {
              !existingRating && handleHover(i);
            }}
            onClick={() => {
              !existingRating && handleClick(i);
            }}
          >
            <StarType fill={existingRating ? i < existingRating : i < rating} />
          </li>
        ))}
      </ul>
    </div>
